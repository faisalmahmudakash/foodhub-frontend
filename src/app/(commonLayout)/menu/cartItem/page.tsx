...
import { placeOrderOnServer } from "@/helpers/cartApi";
...
export default function CartItemPage() {
  ...
  const handleCheckout = async () => {
    ...
    if (!customerId) {
      setCheckoutError("Please log in to place your order.");
      return;
    }
    ...
    setCheckingOut(true);
    try {
      // এই কলটাই আসলে POST /order হিট করে → orderService.createOrder চালায়
      // (CartItem → OrderItem + OrderItemAddon, তারপর CartItem ডিলিট)
      await placeOrderOnServer(customerId);

      // server confirm করার পরই local cart clear করছি
      clearCart();
      setOrderSuccess(true);
    } catch (err: any) {
          ...
          {orderSuccess ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 px-4 py-10 text-center sm:px-6 sm:py-14">
              ...
              <button
                ...
                className="mt-2 rounded-full bg-[#e85d04] px-5 py-2 text-sm font-bold text-white"
              >
                Continue ordering
              </button>
            </div>
            ...
            <>
              ...
              <div className="shrink-0 rounded-b-2xl border-t border-[#ede5d8] bg-white px-4 pt-3 pb-4 sm:px-6 sm:pt-4 sm:pb-6">
                ...
                {checkoutError && (
                  <div className="mb-3 rounded-lg bg-[#fde8d8] px-3 py-2 text-sm font-semibold text-[#e85d04]">
                    {checkoutError}
                  </div>
                )}
